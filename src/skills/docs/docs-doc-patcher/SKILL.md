...
- **User wants to create documentation from scratch** -> redirect to `/docs-doc-router`
- **User wants a full engineering guide** -> redirect to `/docs-engineering-guide-writer`
- **User wants a full test plan** -> redirect to `/docs-test-plan-writer`
- **User wants to initialize a test coverage register** -> redirect to `/docs-coverage-writer`
- **User wants to write or update a spec** -> redirect to `/docs-spec-writer`
- **User has no code changes, just wants to improve doc quality** -> not this skill; suggest manual editing or `/docs-engineering-guide-writer` for a fresh pass
...
**Special cases:**
- **Spec-summary:** if the spec was patched, regenerate the summary by invoking `/docs-spec-summarizer` — spec-summary is always regenerated, never patched directly
...
- **Maintain `.doc-map.yaml`.** If a patch creates a new doc-code relationship (e.g., the diff adds a new module and docs-doc-patcher updates the eng-guide to cover it), update `.doc-map.yaml` to reflect it.
