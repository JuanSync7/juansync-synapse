...
The register is initialized once by this skill and maintained incrementally by `/docs-doc-patcher` as tests are added or code changes. It should never require full regeneration on every change.
...
**Announce at start:** "I'm using the docs-coverage-writer skill to generate the test coverage register."

## Wrong-Tool Detection

- **User wants a test planning document** (how to structure tests in code) → redirect to `/docs-test-plan-writer`
- **User wants actual test code** (pytest, bats, etc.) → redirect to `/code-test-writer`
- **User wants to update existing docs after a code change** → redirect to `/docs-doc-patcher`
...
Test files (evidence)          ← source of "what is tested"
                  ↓
        docs-coverage-writer    ← YOU ARE HERE (cross-matches intent vs evidence)
                  ↓
        test-coverage.md       ← the register (initialized here, maintained by /docs-doc-patcher)
```
...
"Cannot generate coverage register — no intent document found. The register maps what SHOULD be tested to what IS tested. Without a spec, eng-guide, or documented behavioral intent, there is no 'should' to map against. Consider running `/docs-spec-writer` or `/docs-engineering-guide-writer` first."
...
- **Updated incrementally** by `/docs-doc-patcher` when tests are added, modified, or removed
- **Staleness flags refresh** on each generation — they are computed, not stored

When `/docs-doc-patcher` updates this register, it should:
