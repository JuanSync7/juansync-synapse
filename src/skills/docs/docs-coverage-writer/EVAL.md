# docs-coverage-writer — Evaluation Criteria

## Structural Criteria

Delegated to synapse-skill-improver baseline checklist (frontmatter validation, wrong-tool detection, companion file references, etc.).
...
**Why this tests the skill:** This needs `/docs-test-plan-writer` (test planning), not coverage assessment. Tests wrong-tool detection for the closest sibling skill.
...
**Why this tests the skill:** This needs `/code-test-writer` (test code implementation). Tests wrong-tool detection — the trigger keywords ("acceptance criteria", "test") overlap heavily with this skill's domain.
