---
name: docs-engineering-guide-writer
...
Layer 2: Spec Summary            ← docs-spec-summarizer
Layer 3: Authoritative Spec      ← docs-spec-writer
Layer 4: Implementation Plan     ← write-impl / code-build-planner
...
- **User wants a spec** → redirect to `/docs-spec-writer`
- **User wants test planning** → redirect to `/docs-test-plan-writer`
- **User wants pre-implementation docs** → redirect to `/docs-implementation-writer`
...
>
> **Exemption rule:** Skip this gate ONLY when invoked in Phase C of `code-build-planner` with an upstream Phase B (implement-code) marked complete in the same session. In every other case — standalone invocation, orchestrator dispatch, subagent invocation — run the gate.
...
If the subsystem is small and fewer than 3 source files exist, the system may be too trivial to warrant an engineering guide — redirect the user to `/docs-design-writer` or ask whether a brief section in an existing guide would suffice.
...
The docs-engineering-guide-writer skill is post-implementation only. The following paths do not resolve to implemented source files:
...
- If you need pre-implementation design rationale, invoke /docs-design-writer instead.
...
When invoked directly by the user (not from `code-build-planner`), use the Planning Stage + wave execution model below. This mirrors Phase C of `code-build-planner` — the same context isolation applies regardless of invocation source.
...
**docs-engineering-guide-writer section dependency graph:**
...
Wave 1 (parallel — one entry per module):
  module_<name> → One Module Reference section per source module
                  ...
                  Configuration, Error behavior — Test guide is in docs-test-plan-writer)
...
### Parallel mode (Phase C of code-build-planner)
Used when invoked from `code-build-planner`'s Phase C. Two sub-phases:
...
Each agent writes one module section document (5 sub-sections: Purpose, How it works, Key decisions, Configuration, Error behavior — Test guide is in docs-test-plan-writer) and saves it to `docs/tmp/module-<name>.md`. Returns the file path on completion.
...
> **Test planning documentation** (testability maps, mock boundaries, critical scenarios) belongs in `docs-test-plan-writer`, which reads this guide as its primary input.
...
> **Test planning documentation** (testability maps, mock boundary catalogs, critical test scenarios) is NOT part of this guide. Invoke `docs-test-plan-writer` after this document is complete — it reads this guide as its primary input.
...
> **Test guide sub-section is NOT part of the engineering guide.** Test planning belongs in `docs-test-plan-writer`, which reads the Error behavior sub-section to derive test cases.
...
- [ ] No Test guide sub-section present — test planning belongs in docs-test-plan-writer
