...
- **User wants to write a full spec from scratch** → redirect to `/docs-spec-writer`
- **User wants a design doc** → redirect to `/docs-design-writer`
...
Layer 3: Authoritative Spec     ← docs-spec-writer (required input — must exist)
...
**Why this stage exists:** docs-spec-summarizer is a short document (150-250 lines), so per-section subagent dispatch would add disproportionate overhead. However, scoping which spec sections to read per output section still improves quality by preventing the agent from holding the full spec in active context for every section.
...
### docs-spec-summarizer section-to-source mapping
...
- `docs-spec-writer` — the companion spec must exist before writing a summary
...
> "Spec summary complete and saved to `[path]`. Next step: `/write-design` to begin task decomposition, or update the summary later with `/docs-spec-summarizer` when the spec changes."

## Document Chain

```
Unphased:
docs-spec-writer  →  docs-spec-summarizer  →  write-design  →  code-build-planner
 _SPEC.md            _SPEC_SUMMARY.md       _DESIGN.md       _IMPLEMENTATION.md

Phased:
docs-spec-writer  →  docs-spec-summarizer  →  write-design  →  docs-implementation-writer
