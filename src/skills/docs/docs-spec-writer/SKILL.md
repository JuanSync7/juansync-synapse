...
- **User wants a quick summary of an existing spec** → redirect to `/docs-spec-summarizer`
- **User wants a design doc with task decomposition** → redirect to `/docs-design-writer`
- **User wants a surgical update to a specific section from a git diff** → redirect to `/docs-doc-patcher`
...
Layer 5: Implementation Plan    ← code-build-planner
```
...
- `docs-spec-summarizer` — generate a concise digest of this spec
...
> "Spec complete and saved to `[path]`. Companion map at `[map_path]`. Next steps: `/docs-spec-summarizer` for a concise digest, or `/write-design` to begin task decomposition. Which would you like?"
