...
Phase E: Test Implementation     ← code-test-writer (reads this document)
```

**Required inputs:**
- Engineering guide (Phase C) — must exist before writing
- Phase 0 contracts from docs-implementation-writer — for import surface and error taxonomy
...
**Announce at start:** "I'm using the docs-test-plan-writer skill to create the test planning document."
...
2. **Phase 0 contracts path** — from docs-implementation-writer. For import surface and error taxonomy. Do NOT read source files.
...
> **For code-test-writer agents:** This document is your source of truth.
...
**Spec:** [path]
**Produced by:** docs-test-plan-writer
...
One section per module from the engineering guide. Each section is self-contained — the code-test-writer agent reads only this section.
...
#### Agent isolation contract (include verbatim in code-test-writer dispatch)
...
- `docs-engineering-guide-writer` — must exist
- `docs-implementation-writer` — for Phase 0 contracts and error taxonomy

**Downstream (invoke after this skill):**
- `code-test-writer` — reads this document module-by-module to implement pytest code

**Document Chain:**

```
docs-implementation-writer → implement-code → docs-engineering-guide-writer
                                                       ↓
...
> "Test docs complete and saved to `[path]`. Invoke `code-test-writer [module-name] [path-to-this-doc]` per module section to implement pytest coverage."
