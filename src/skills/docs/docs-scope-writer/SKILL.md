...
- **Tech stack or component decisions** (databases, frameworks, service boundaries) --> redirect to `/docs-architecture-writer`
- **Formal requirements with FR/NFR IDs and acceptance criteria** --> redirect to `/docs-spec-writer`
- **User wants to implement or write code** --> scope should be finalized first. Say: "Finalize scope before implementation. Run `/docs-scope-writer` checkpoint to verify readiness."
...
3. **If confirmed ready:**
   ...
   - Hand off: "Scope finalized. Invoke `/docs-spec-writer` to formalize requirements or `/docs-architecture-writer` for technical architecture."
...
- **docs-spec-writer** reads: Goals, Scope Boundary (In Scope items become requirements candidates), Phase Plan (which phase is being specified)
- **docs-architecture-writer** reads: Goals, Scope Boundary, Phase Plan, Key Decisions (technology choices inform architecture)
