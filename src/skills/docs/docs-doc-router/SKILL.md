...
- **User wants to improve an existing skill** → redirect to `/synapse-skill-improver`
- **User already knows which doc type** → invoke that skill directly (`docs-spec-writer`, `docs-design-writer`, etc.)
...
```
Layer 0   — Scope                  -> invoke /docs-scope-writer
               |
               v
Layer 0.5 — Architecture           -> invoke /docs-architecture-writer
               |
               v
Layer 1   — Platform Spec          (written manually — no skill)
               |
               v
Layer 2   — Spec Summary           -> invoke /docs-spec-summarizer
               |
               v
Layer 3   — Authoritative Spec     -> invoke /docs-spec-writer
               |
               v
Layer 4   — Implementation Plan    -> invoke /write-impl
               |
               v
Layer 5   — Engineering Guide      -> invoke /docs-engineering-guide-writer
              (post-implementation)
               |
               v
Layer 6   — Module Tests           -> invoke /code-test-writer or /docs-test-plan-writer
```

| User Intent | Skill to Invoke |
|-------------|----------------|
| Define what to build, phase planning, scope decisions | `/docs-scope-writer` |
| Technology choices, component structure, data flow patterns | `/docs-architecture-writer` |
| Summarize an existing spec | `/docs-spec-summarizer` |
| Sync a summary with an updated spec | `/docs-spec-summarizer` (update mode) |
| Write requirements / a new spec | `/docs-spec-writer` |
| Write an implementation plan (pre-build) | `/write-impl` |
| Document what was built / post-implementation reference | `/docs-engineering-guide-writer` |
| Test team handoff / maintenance reference | `/docs-engineering-guide-writer` |
