...
- **What to build/not build, phase planning** --> redirect to `/docs-scope-writer`
- **Formal requirements with acceptance criteria** --> redirect to `/docs-spec-writer`
- **Task decomposition and code contracts** --> redirect to `/docs-design-writer`
- **Post-implementation documentation** --> redirect to `/docs-engineering-guide-writer`
...
```
Layer 0:   Scope Document         <-- docs-scope-writer (input to this skill)
Layer 0.5: Architecture Document  <-- YOU ARE HERE
Layer 3:   Authoritative Spec     <-- docs-spec-writer (consumes this)
Layer 4:   Design Document        <-- docs-design-writer (consumes this)
```

**Announce at start:** "I'm using docs-architecture-writer to [create/update/checkpoint] the architecture document."
...
3. **If user confirms:**
   ...
   - Hand off: "Architecture finalized. Invoke `/docs-spec-writer` to begin formal requirements, or `/docs-design-writer` if you already have a spec."
...
- **`docs-spec-writer`** reads the architecture for technology decisions and component boundaries when writing formal requirements
- **`docs-design-writer`** reads the architecture for component structure when decomposing tasks into implementation units
