...
description: Use when you have a spec and need to create a design document with task decomposition and code contracts before implementation. Triggered by "write design docs", "docs-design-writer", "design document", "create design", "technical design".
domain: docs
scope: design
role: writer
...
```
Layer 3: Authoritative Spec     ← docs-spec-writer (required input)
Layer 4: Design Document        ← YOU ARE HERE
Layer 5: Implementation Docs    ← docs-implementation-writer (consumes this document)
...
- **User wants a spec (requirements, acceptance criteria)** → redirect to `/docs-spec-writer`
- **User wants implementation-level task details** → redirect to `/docs-implementation-writer`
- **User wants to jump straight to code** → redirect to `/code-build-planner` or code directly
...
**Downstream contract:** `docs-implementation-writer` copies Part B CONTRACT entries verbatim into Phase 0. `implement-code` receives PATTERN entries directly — they never appear in implementation docs or reach test agents.

**Announce at start:** "I'm using docs-design-writer to create the technical design document."
...
- `docs-spec-writer` — the spec (Layer 3) must exist. Extract all REQ/FR IDs before writing.

**Downstream (in order):**
1. `docs-implementation-writer` — copies Part B CONTRACT entries into Phase 0, writes isolated task sections
2. `implement-code` — receives PATTERN entries directly (never through implementation docs)
3. `docs-engineering-guide-writer` — documents what was built
4. `docs-test-plan-writer` → `code-test-writer`

**Document chain:**
```
docs-spec-writer → docs-design-writer → docs-implementation-writer
                                        ↓
                                  implement-code
                                        ↓
                              docs-engineering-guide-writer
                                        ↓
                                 docs-test-plan-writer
                                        ↓
                               code-test-writer
```
...
> "Design document complete and saved to `[path]`. Next step: invoke `/docs-implementation-writer [system] [spec path] [this design doc path]` to create the implementation source-of-truth."
