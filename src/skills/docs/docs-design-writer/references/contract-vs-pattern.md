...
|------------|----------|-------------------------------|
| **CONTRACT** | docs-implementation-writer Phase 0 → all agents | If a pattern is labelled as contract, implement-code agents receive implementation hints in their Phase 0 — biasing them toward one approach and compromising test agents who should derive tests from contracts alone |
| **PATTERN** | implement-code task agents directly | If a contract is labelled as pattern, docs-implementation-writer Phase 0 is missing types/stubs — agents can't build against incomplete interfaces |
