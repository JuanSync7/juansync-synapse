...
description: Use when you have a spec AND a design doc and need to produce the implementation source-of-truth before touching code. Triggered by "write implementation docs", "docs-implementation-writer", "impl docs", "create implementation reference", "implementation source of truth".
domain: docs
...
- **User wants a design doc (task decomposition, contracts)** → redirect to `/docs-design-writer`
- **User wants an execution plan for parallel agents** → redirect to `/code-build-planner`
...
**Announce at start:** "I'm using the docs-implementation-writer skill to create the implementation source-of-truth."
...
## Module Boundary Map                   ← task → source files (feeds docs-engineering-guide-writer)
...
3. `docs-engineering-guide-writer` — module boundary map tells it which source files to document
4. `docs-test-plan-writer` — reads engineering guide + Phase 0 contracts from this document
5. `code-test-writer` — reads docs-test-plan-writer module sections to implement pytest code

**Document chain:**
```
docs-spec-writer → write-design → docs-implementation-writer
                                     ↓
                               implement-code
                                     ↓
