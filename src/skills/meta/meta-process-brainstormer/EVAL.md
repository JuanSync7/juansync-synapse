# meta-process-brainstormer — Evaluation Criteria

> **Execution scope:** This file is ignored during skill execution — it is read only by `/synapse-skill-improver` and evaluation workflows.

## Structural Criteria

Covered by `/synapse-skill-improver` baseline checklist — not duplicated here. Structural checks include: frontmatter validity, domain/intent in TAXONOMY, companion file presence, SKILL.md length under 500 lines, Wrong-Tool Detection section, Execution Scope fence.
...
### EVAL-O08
**Criterion:** When the skill receives a factual question (e.g., "What does TCP stand for?"), it does not run the meta-process-brainstormer protocol and instead redirects or answers directly.
...
**Rationale:** Catches wrong-tool false positive where the skill fires on inputs that should never enter the meta-process-brainstormer protocol.

### EVAL-O09
**Criterion:** When the input is a skill-shaped topic (mentions "skill," "SKILL.md," slash command, or agentic behavior), the coach redirects to `/synapse-router-artifact-brainstormer` without running the meta-process-brainstormer protocol.
**Pass:** The response contains a redirect to `/synapse-router-artifact-brainstormer` and no Phase A work is performed.
...
### EVAL-O15
**Criterion:** The lens selection in the session consists of exactly 4 lenses: 2 universal (Stakeholder + Alternative) plus 2 type-specific lenses from the lens catalog for the classified meta-process-brainstormer type.
...
### EVAL-O20
**Criterion:** When the meta-process-brainstormer type drifts mid-session (e.g., from `decision` to `reframe`), `meta.yaml` is updated to reflect the new `(type, shape)` and the notepad contains a note recording the shift.
...
### Naive (should invoke meta-process-brainstormer)
...
### Experienced (should invoke meta-process-brainstormer)
...
### Adversarial (should NOT invoke meta-process-brainstormer — skill must refuse/redirect)
- "what's the capital of France and also the year the Eiffel Tower was built" — expected: direct answer, no meta-process-brainstormer
...
- "I have an idea for a skill that helps developers write better commit messages — I'm not sure if it should be one skill or two, or what the phases would look like" — expected: `/synapse-router-artifact-brainstormer`
...
- "should I add a wrong-tool detection section to this skill I'm writing, or is the description enough to prevent misfires?" — expected: `/synapse-router-artifact-brainstormer`
