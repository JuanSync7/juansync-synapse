...
Governs coaching behavior across every phase of a meta-process-brainstormer session. Load at session start.
...
If the topic shifts mid-session (e.g., a decision-type meta-process-brainstormer becomes a reframe — "the real question is whether we need this at all"), update `(type, shape)` in `meta.yaml`, adjust the lens subset to match, and note the shift in the notepad. Not a bug; an explicit capability.
