...
The mechanism that makes "enough for today — pick up tomorrow" possible. Without this, meta-process-brainstormer sessions spiral indefinitely. The circuit breaker has two parts: a **diagnostic** (when to fire) and an **action** (the `Pause` move).
...
> "Paused. Notepad saved at `<path>`. Threads still open: T3 (needs profiling data), T7 (needs input from Sarah). When you're ready, invoke `/meta-process-brainstormer` and I'll offer to resume."
...
On next invocation of `/meta-process-brainstormer`:
...
> "Existing meta-process-brainstormer on `<topic>` from `<date>`, paused. Open threads: T3, T7. Resume, or start fresh?"
