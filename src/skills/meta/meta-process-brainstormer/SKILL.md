...
- **User has a clear spec/plan/direction already** (nothing left to decide) → redirect to the appropriate implementation skill. *Direction-based, not verb-based: "build a dashboard" with no direction IS a meta-process-brainstormer; "implement this spec" with a concrete spec is not.*
- **Factual question with a definite answer** → answer directly; do not meta-process-brainstormer
- **Specific concrete bug with a clear failure trace** → debug directly; meta-process-brainstormer only fires when root cause is architectural
- **End-to-end pipeline request** ("meta-process-brainstormer → spec → code → docs") → redirect to `/autonomous-orchestrator`
...
## Coexistence with Built-In `meta-process-brainstormer` Stage

`SKILLS_REGISTRY.yaml` declares a built-in `meta-process-brainstormer` stage executed internally by `/autonomous-orchestrator`. That stage and this user-invocable skill share a name but not an implementation. If invoked by the orchestrator, the built-in stage runs; if invoked by the user via `/meta-process-brainstormer`, this skill runs.
...
Load: references/wrong-tool-detection.md, references/meta-process-brainstormer-types.md
...
### [X] Early exit (no meta-process-brainstormer needed)
...
Don't:
  - Produce a full memo for a non-meta-process-brainstormer outcome
