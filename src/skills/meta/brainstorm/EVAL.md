...
> **Execution scope:** This file is ignored during skill execution — it is read only by `/synapse-skill-skill-improver` and evaluation workflows.

## Structural Criteria

Covered by `/synapse-skill-skill-improver` baseline checklist — not duplicated here. Structural checks include: frontmatter validity, domain/intent in TAXONOMY, companion file presence, SKILL.md length under 500 lines, Wrong-Tool Detection section, Execution Scope fence.
...
### EVAL-O09
**Criterion:** When the input is a skill-shaped topic (mentions "skill," "SKILL.md," slash command, or agentic behavior), the coach redirects to `/synapse-router-artifact-brainstormer` without running the brainstorm protocol.
