...
distinction signals confidence, not severity in the runtime sense. `test-fix` uses these
...
Do not delete or bypass it — `test-fix` can add entries when actionable suppression is
needed.
...
Only entries in the whitelist file suppress a finding. Remind `test-fix` of this if it
...
`test-fix` will action these findings by either deleting the dead symbol or adding it to
the whitelist if the removal is unsafe. The test-lint skill's responsibility is only to
