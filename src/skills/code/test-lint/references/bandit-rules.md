...
Loaded at [RUN-EACH-LINTER]. Covers bandit's scope, severity mapping, invocation, config, and suppression handling for the test-lint skill.
...
Confidence is metadata, not a filter. Preserve it in `LintIssue.message` so test-fix and human reviewers can weigh LOW-confidence findings appropriately. Example message format: `[B301] Use of pickle.loads (confidence: HIGH)`.
...
- This surfaces suppressions for test-fix to review whether each is justified. Bandit still skips the suppressed line — the `NOSEC_PRESENT` issue is an audit flag only, not a re-surfaced security finding.

---

## Common False-Positive Patterns

Bandit surfaces all findings. The skill does not filter. These patterns are known to produce frequent false positives — document them so test-fix and reviewers have context:
...
Surface all findings as-is. If a finding is a false positive for the project, test-fix or a human reviewer should add the rule to `skips` in the bandit config — not add `# nosec` inline, and not filter at aggregation time.
