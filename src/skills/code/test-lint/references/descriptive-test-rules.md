...
`test-lint` skill validates this contract at [RUN-EACH-LINTER]; `test-generate` enforces
...
Used to trace a test back to the `test-generate` run that produced it, enabling audit
...
- **`test-fix`** reads `descriptive_test_violations` to add or correct missing tags without
  touching unrelated lint issues.
- **`test-generate`** blocks on any `MISSING_DOCSTRING` or `MISSING_*_TAG` violation in
