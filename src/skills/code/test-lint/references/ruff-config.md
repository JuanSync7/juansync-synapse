...
If the project has no ruff config and you observe high E501 noise in test files, record it as a `LintIssue` with a note that `per-file-ignores` configuration is missing — let `test-fix` decide whether to add it.
