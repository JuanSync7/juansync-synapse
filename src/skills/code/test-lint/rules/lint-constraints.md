...
Hard constraints for `test-lint`. Loaded at every node. No exceptions.
...
- MUST surface all fixable findings as `LintIssue` entries — fixing is `test-fix`'s responsibility, not this skill's.
