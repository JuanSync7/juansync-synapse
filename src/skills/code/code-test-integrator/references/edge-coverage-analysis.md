...
- The warning is **advisory only** at the code-test-integrator stage. Don't block; don't revert. Track in `EDGE_COVERAGE.yaml` audit log for periodic review.
...
- Computing edge coverage from scratch — that's `code-test-auditor`'s job.
- Hard-gating coverage thresholds — `code-test-auditor` and `code-test-evaluator` decide what's worth promoting; this stage is post-merge feedback.
