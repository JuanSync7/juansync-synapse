...
Loaded at `[PICK-PATTERN]`. Canonical pattern table — must match `code-test-evaluator` verbatim. Assign exactly one pattern per strategy item.
...
8. If the strategy item's `recommended_pattern` (from code-test-evaluator) conflicts with these rules, abort the item and log `pattern-mismatch` rather than improvise.
