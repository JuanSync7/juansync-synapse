...
- **No `IntegrationStrategy` documents on disk** → `/test-evaluate` first, then return here
- **User wants the classification, not the conversion** → `/test-evaluate`
- **User wants to write fresh mock-integration tests** → `/test-generate`
