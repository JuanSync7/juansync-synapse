...
Evaluation criteria for the `test-integrate` skill at `/home/juansync7/juansync-synapse/src/skills/code/test-integrate/SKILL.md`.
...
### EVAL-T02
**Persona:** Naive User
**Prompt:** "I have an IntegrationStrategy from test-evaluate — make the real tests now"

### EVAL-T03
**Persona:** Naive User
**Prompt:** "run test-integrate on item DB-001 only"
...
### EVAL-O14
No local declaration of `IntegrationStrategy`, `CoverageState`, `FlakinessSummary`, or `EdgeCoverageReport` exists outside `ai-synapse/tools/testing/schemas.py`; all imports resolve to that path.
...
### EVAL-E11
Wrong-tool detection redirects fire BEFORE [NEW] step 1: requests to classify mocks → `/test-evaluate`; requests to write fresh mock tests → `/test-generate`; requests to lint → `/test-lint`. No partial execution.
