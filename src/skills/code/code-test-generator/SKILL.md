...
- **User wants to run/execute existing tests** → `/code-test-runner`
- **User wants to evaluate test quality of existing suite** → `/code-test-evaluator`
- **User wants to fix lint findings** → `/code-test-fixer`
...
### [END]
Do: Print PR URL (or intent-list path with `--no-pr`), per-gap result table (closed / unresolvable / mutation-survived / score-deficit / implementation-coupled / human-rejected), Hypothesis ratio achieved, and benchmark candidate count. Suggest `/code-test-evaluator` once the PR is reviewed and merged.
Don't: Auto-route or invoke `/code-test-evaluator` directly — handoff is the user's call.
