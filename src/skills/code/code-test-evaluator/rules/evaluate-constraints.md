# code-test-evaluator constraints

Hard normative invariants for the `code-test-evaluator` skill. Always loaded. All rules are hard unless explicitly marked `(soft)`.
...
Never start containers, databases, queues, HTTP servers, or any runtime dependency during evaluation. This skill is static analysis only; live probing belongs to `code-test-integrator`. Reject any plan step that calls `docker`, `compose`, `testcontainers`, server bootstraps, or network connects.
...
The engine does not block on reviewer response. After emitting strategies, open the PR and continue to `code-test-integrator`. Reviewer feedback is async and applied on the next rerun. This contrasts with `code-test-generator`, which hard-gates on reviewer approval before touching test code.
...
`IntegrationStrategy` documents are plain English plus the structured ranked-candidate table. No test code, no diffs, no fixture skeletons, no import stubs, no pytest decorators. Code generation is `code-test-integrator`'s job. If a recommendation cannot be expressed in prose, it does not belong here.

## 10. No mock quality judgement

Mock correctness, vacuous-assertion detection, assertion strength, and overall test quality are out of scope — `code-test-linter` and `code-test-auditor` own those. This skill classifies boundary tier and recommends a lifecycle pattern. Do not flag weak mocks, brittle assertions, or stylistic test issues; ignore them even when obvious.
