...
- **User has no `CoverageState` on disk** → `/code-test-generator` first, then return here
- **User wants to write the real-integration tests** → `/code-test-integrator` (this skill only emits the strategy)
- **User wants to evaluate test code quality / lint** → `/code-test-linter`
- **User wants to find coverage gaps** → `/code-test-auditor`
...
Don't: Auto-route or invoke `/code-test-integrator` directly — handoff is the user's call.
