...
- **User wants a fresh lint scan** → `/code-test-linter` (produces the LintReport this skill consumes)
- **User wants test coverage gaps audited** → `/code-test-auditor` (downstream, requires lint-clean state)
