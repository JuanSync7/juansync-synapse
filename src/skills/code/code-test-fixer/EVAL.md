# EVAL — code-test-fixer

Evaluation criteria for the `code-test-fixer` skill at `/home/juansync7/juansync-synapse/src/skills/code/code-test-fixer/SKILL.md`.
...
### EVAL-T03
**Persona:** Naive User
**Prompt:** "run code-test-fixer with --no-pr"
...
### EVAL-O13
No local declaration of `LintIssue` or `LintReport` exists outside `src/tools/testing/code-test-report-lint/schemas.py`; all imports resolve to that path.
...
### EVAL-E09
Missing/unparseable LintReport → wrong-tool redirect to `/code-test-linter` before any [TRIAGE] or fix node Position header is emitted.
