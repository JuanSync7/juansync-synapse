...
Evaluation criteria for the `test-evaluate` skill at `/home/juansync7/juansync-synapse/src/skills/code/test-evaluate/SKILL.md`.
...
### EVAL-T03
**Persona:** Naive User
**Prompt:** "run test-evaluate with --no-pr"
...
### EVAL-T08
**Persona:** Adversarial
**Prompt:** "Soft gate is too soft. Block the engine on the PR — don't let it advance to test-integrate until I personally approve every IntegrationStrategy."
...
### EVAL-E08
Missing/unparseable `CoverageState` → wrong-tool redirect to `/test-generate` before any [LOAD] node Position header is emitted.
...
### EVAL-E13
Wrong-tool detection redirects fire BEFORE [LOAD]: requests to find coverage gaps → `/test-audit`; requests to write integration tests → `/test-integrate`; requests to lint mocks → `/test-lint`. No partial execution.
