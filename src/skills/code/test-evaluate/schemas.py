"""
@summary
Canonical pydantic v2 contracts for the `test-evaluate` skill.

This module is the source-of-truth for two cross-stage shapes used by the
6-skill test coverage engine:

- ``CoverageState`` — produced by ``/test-generate``, consumed by
  ``/test-evaluate`` (and downstream stages). Captures per-module test
  coverage status, including whether mocked-integration tests exist and
  the priority ranking used to drive evaluation order.
- ``IntegrationStrategy`` — produced by ``/test-evaluate`` per module.
...
class IntegrationStrategy(BaseModel):
    """Per-module integration strategy emitted by `/test-evaluate`.
...
class ModuleCoverage(BaseModel):
    ...
    # Populated by `/test-evaluate` after a run.
...
class AuditGapReport(BaseModel):
    """Gap report emitted by `/test-audit`; consumed by `/test-evaluate`."""
...
class CoverageState(BaseModel):
    ...
    Produced by `/test-generate`, mutated by `/test-evaluate` (per-module
    strategy paths + boundary classification + run id + source hash), and
    consumed by downstream stages (`/test-integrate`, `/test-lint`, etc.).
    """
