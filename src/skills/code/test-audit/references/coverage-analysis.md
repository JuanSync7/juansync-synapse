...
| **Mutation kill rate** | Weak or missing assertions | `mutation_runner` in `test-generate` (per-gap) |
...
`test-generate`, not here. Edge coverage is computed later at [EDGE-COV] via static AST walk and
...
(`test-generate`) what style of test is needed. Full layer definitions live in
...
For real-infrastructure tests (testcontainers, vcrpy), classification happens in `test-evaluate`,
not here. Mark those as `mock-integration` at [SNAPSHOT] and let [EDGE-COV] and `test-evaluate`
refine.
...
- Do not run mutation testing — that belongs to `test-generate`.
