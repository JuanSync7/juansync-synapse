...
This is a **recommendation signal only.** Audit emits the signal; `test-integrate` owns the
...
3. Do NOT trigger an inline reruns campaign. Deciding whether and how to collect flakiness
   history is `test-integrate`'s responsibility (open question tracked in the decision memo).
...
> emits the raw scores; human or `test-integrate` interprets the pattern.
