...
Read-only first stage of the 6-skill test coverage engine. Orchestrates 9 testing tools sequentially to snapshot coverage, score criticality, detect gaming, and produce `AuditGapReport` — the canonical input that `test-generate`, `test-evaluate`, and `test-integrate` all consume.
...
- Run mutation testing inline (that is `test-generate`'s per-gap responsibility)
- Quarantine, delete, or auto-fix any flagged test

## Wrong-Tool Detection
- **User wants tests written from gaps** → `/test-generate` (consumes this skill's output)
- **User wants to fix lint issues** → `/test-fix` (audit's precondition)
- **User wants to evaluate test boundary correctness** → `/test-evaluate`
...
### [END]
Do:
  1. Emit `AuditGapReport` for downstream consumers (`test-generate` / `test-evaluate` / `test-integrate`).
  2. Print summary: total gaps, critical-tier count, gaming alerts, flakiness candidates, edge gaps, log violations, dep vulns.
  3. Suggest: "Run `/test-generate` to write tests for prioritized gaps."
Don't: Auto-route to `/test-generate`.
