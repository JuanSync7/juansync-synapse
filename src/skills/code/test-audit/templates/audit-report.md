...
> handed off to downstream skills (`test-generate`, `test-evaluate`, `test-integrate`).
...
Delta computation (new gaps vs prior run) is a consumer responsibility — `test-generate`
and `test-evaluate` apply delta filtering using `AUDIT_HISTORY/` if needed.
