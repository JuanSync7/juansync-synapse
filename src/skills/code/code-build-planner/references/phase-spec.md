# Phase Specification — Six-Phase Bias-Free Plan

Loaded at `[DERIVE]` and `[DRAFT]`. This is the authoring detail for each phase the plan document must contain. SKILL.md carries the flow; this file carries the per-phase contracts.
...
Layer 4: Design Document        ← docs-design-writer
Layer 5: Implementation Docs    ← docs-implementation-writer (required input)
Layer 6: Build Plan             ← YOU ARE HERE (code-build-planner)
```

**Input requirements:**
1. An implementation docs document (from `docs-implementation-writer`) — the source of truth for contracts, task sections, and the dependency graph.
...
**Invoke:** `docs-engineering-guide-writer` skill for both sub-phases.
...
**Invoke:** `code-test-writer` skill per task.
