...
- MUST NOT execute mutation testing inline — mutation testing is `code-test-generator`'s per-gap responsibility.
...
- MUST require a lint-clean precondition before proceeding past [NEW] — lint passes or all `LintReport.issues` are flagged `requires-human-review`; otherwise abort and direct the user to run `/code-test-fixer` first.
