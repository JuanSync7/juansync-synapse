# EVAL — `code-test-auditor`

Evaluation specification for the `code-test-auditor` skill. Three independent surfaces: blind test prompts (T), output quality criteria (O), execution criteria (E).
...
**Expected:** should-redirect (to `/code-test-generator` or `/code-test-writer`)
...
- [ ] **EVAL-O12:** Lint-clean precondition checked before any tool executes
  - **Test:** Inject deliberate lint failure; invoke audit. Run aborts at [NEW] with directive to run `/code-test-fixer`; no tool output, no `AUDIT_HISTORY` entry.
...
- [ ] **EVAL-E12:** [END] does not auto-route to `/code-test-generator`
  - **Test:** Summary + suggestion present; no autonomous dispatch of `/code-test-generator`.
  - **Fail:** `/code-test-generator` invoked or test-writing action initiated.
