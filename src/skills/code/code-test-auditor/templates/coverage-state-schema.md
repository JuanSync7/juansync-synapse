...
- NEVER overwrite an existing `COVERAGE_STATE.yaml` — audit writes it once; `code-test-generator` owns it thereafter.
...
Written ONCE on first run (when file does not yet exist). `code-test-generator` and `code-test-integrator`
mutate this file thereafter — audit MUST NOT touch it after initialization.

```yaml
# Living state file. Written once by code-test-auditor; mutated by code-test-generator / code-test-integrator.
...
gap_state:
  ...
  closed: []                         # empty at initialization; code-test-integrator populates
...
flakiness_quarantine: []             # code-test-integrator sets quarantine entries here
generation_lineage: []               # code-test-generator appends generation records here
```
...
project/coverage/
└── state/
    ├── COVERAGE_STATE.yaml               # baseline (written once by code-test-auditor)
    └── AUDIT_HISTORY/
