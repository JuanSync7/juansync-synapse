...
4. **Mock by default.** All external dependencies — network I/O, filesystem, time, subprocess, and database — must be mocked unless the test is explicitly classified as a real-integration test by `test-evaluate`. Never mock the unit under test itself; mocking the SUT invalidates the test.
