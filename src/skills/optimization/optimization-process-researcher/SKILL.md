...
scope: process
role: researcher
tags: [optimization-process-researcher, experiment-loop, optimization]
user-invocable: true
...
- **User wants a one-shot structural pass on an existing skill** (not an iterative loop) → redirect to `/synapse-skill-improver`
...
- **User wants to find a root cause or fix a specific bug** → this is debugging, not optimization; optimization-process-researcher is the wrong shape
- **User has no scorable metric and no way to compare "better"** → optimization-process-researcher can't help; try to surface a dimension with them, or route them elsewhere
...
- **One-shot structural fixes on a skill** — use `/synapse-skill-improver`
...
|------|---------|-------|--------|
| **Setup** | `optimization-process-researcher setup [target path]` | A goal and the user's domain knowledge | PROGRAM.md ready for `run` mode |
| **Run** | `optimization-process-researcher run [path to PROGRAM.md]` | A valid PROGRAM.md | Improved target + research/iterations.tsv + research/changelog.md |
...
3. **Run** — jump straight to `optimization-process-researcher run [path]`, skipping setup entirely
...
| Task shape | Route to |
|------|----------|
| One-shot structural pass on an existing skill | `/synapse-skill-improver [path]` |
...
| Debugging / root-cause analysis | Not optimization-process-researcher — use normal debugging tools |
