...
When synapse-skill-improver fixes a flow-graph SKILL.md, it must preserve the graph topology. Edit within nodes — do not restructure across them. These rules apply during both structural and behavioral fix cycles.
...
synapse-skill-improver references companion files from synapse-router-artifact-creator via symlinks. Before scoring begins, verify all Load targets resolve. If any Load target does not resolve (broken symlink, missing file, dead reference):
