...
Creator skills (`synapse-router-artifact-creator`, `agent-creator`, `protocol-creator`) create new synapses but consistently forget to update the domain README.md with a row for the new artifact. The pre-commit hook catches the missing row, but by then the contributor has to manually figure out the right format and add it.
